...
var WORDS={
  ...
  'bird':'words/bird.mp3','blank':'words/blank.mp3','bland':'words/bland.mp3','blast':'words/blast.mp3','blend':'words/blend.mp3','blink':'words/blink.mp3','blob':'words/blob.mp3','blue':'words/blue.mp3','blunt':'words/blunt.mp3','bolt':'words/bolt.mp3',
  ...
  'draft':'words/draft.mp3','dream':'words/dream.mp3','drift':'words/drift.mp3','drink':'words/drink.mp3','drop':'words/drop.mp3','drum':'words/drum.mp3','duck':'words/duck.mp3','duct':'words/duct.mp3',
  ...
  'pinch':'words/pinch.mp3','pink':'words/pink.mp3','plan':'words/plan.mp3','plank':'words/plank.mp3','plant':'words/plant.mp3','play':'words/play.mp3','plug':'words/plug.mp3','plump':'words/plump.mp3','plunk':'words/plunk.mp3','pond':'words/pond.mp3','pot':'words/pot.mp3',
  ...
  'speak':'words/speak.mp3','spend':'words/spend.mp3','spent':'words/spent.mp3','spin':'words/spin.mp3','splash':'words/splash.mp3','splat':'words/splat.mp3','splint':'words/splint.mp3','split':'words/split.mp3','spot':'words/spot.mp3','spunk':'words/spunk.mp3','sprig':'words/sprig.mp3',
  'spring':'words/spring.mp3','sprint':'words/sprint.mp3','stamp':'words/stamp.mp3','stand':'words/stand.mp3','star':'words/star.mp3','stay':'words/stay.mp3','stilt':'words/stilt.mp3','stink':'words/stink.mp3','stomp':'words/stomp.mp3','step':'words/step.mp3','stop':'words/stop.mp3',
  'store':'words/store.mp3','storm':'words/storm.mp3','strand':'words/strand.mp3','strap':'words/strap.mp3','strict':'words/strict.mp3','strip':'words/strip.mp3','strong':'words/strong.mp3','strum':'words/strum.mp3',
  ...
  'trip':'words/trip.mp3','trump':'words/trump.mp3','trunk':'words/trunk.mp3','trust':'words/trust.mp3','tub':'words/tub.mp3','tusk':'words/tusk.mp3','twig':'words/twig.mp3','twin':'words/twin.mp3',
  'twist':'words/twist.mp3','type':'words/type.mp3','umbrella':'words/umbrella.mp3','unicorn':'words/unicorn.mp3','van':'words/van.mp3','verb':'words/verb.mp3','very':'words/very.mp3',
...
function playAudio(path,el){if(!el||!el.classList)return;if(el.classList.contains('on'))return;var player=new Audio(AUDIO_BASE+path);player.preload='auto';el.classList.add('on');player.onended=function(){el.classList.remove('on')};player.onerror=function(){el.classList.remove('on')};player.play().catch(function(e){el.classList.remove('on')})}
...
function spk(word){var w=word.toLowerCase();var p=WORDS[w];if(p){var player=new Audio(AUDIO_BASE+p);player.preload='auto';var fired=false;var fallback=function(){if(!fired){fired=true;speakNow(w)}};player.onerror=fallback;player.play().catch(fallback);}else{speakNow(w);}}
...
// ═══ STORY CARDS ═══
(function(){
  var demos=[{v:'a',w:'stamp',e:'邮票 ✉️',end:'mp',s1:'/m/',s2:'/p/'},{v:'i',w:'drink',e:'喝 🥤',end:'nk',s1:'/ŋ/',s2:'/k/'},{v:'u',w:'trust',e:'信任 🤝',end:'st',s1:'/s/',s2:'/t/'},{v:'a',w:'plant',e:'植物 🌱',end:'nt',s1:'/n/',s2:'/t/'},{v:'e',w:'blend',e:'混合 🌀',end:'nd',s1:'/n/',s2:'/d/'},{v:'o',w:'frost',e:'霜 ❄️',end:'st',s1:'/s/',s2:'/t/'},{v:'a',w:'blast',e:'爆炸 💥',end:'st',s1:'/s/',s2:'/t/'},{v:'i',w:'swift',e:'快速 💨',end:'ft',s1:'/f/',s2:'/t/'},{v:'a',w:'scalp',e:'头皮 💆',end:'lp',s1:'/l/',s2:'/p/'},{v:'u',w:'skulk',e:'躲藏 🕵',end:'lk',s1:'/l/',s2:'/k/'}];
  var h='';
  demos.forEach(function(d){h+='<div style="background:rgba(206,147,216,.04);border:1px solid rgba(206,147,216,.15);border-radius:12px;padding:9px 12px;text-align:center;min-width:80px">';h+='<div style="font-size:1.1em;font-weight:bold;color:#ffd200;letter-spacing:2px;margin-bottom:2px">'+d.w+'</div>';h+='<div style="display:flex;gap:1px;justify-content:center;align-items:center;margin-bottom:3px;flex-wrap:wrap">';h+='<span style="font-size:.7em;color:#888">-'+d.end+':</span>';h+='<span style="color:#ef9a9a;font-weight:bold;cursor:pointer;font-size:.8em" onclick="spkPhoneme(\''+d.s1+'\',this)">'+d.end[0]+' 🔊</span>';h+='<span style="color:#888;font-size:.6em">+</span>';h+='<span style="color:#ef9a9a;font-weight:bold;cursor:pointer;font-size:.8em" onclick="spkPhoneme(\''+d.s2+'\',this)">'+d.end[1]+' 🔊</span>';h+='</div>';h+='<div style="font-size:.6em;color:#aaa">'+d.e+'</div>';h+='<span class="speak-btn" onclick="spk(\''+d.w+'\')" style="display:inline-block;margin-top:2px;width:24px;height:24px;font-size:.55em">🔊</span></div>'});
  document.getElementById('ccvccVariety').innerHTML=h;
})();
(function(){
  var c=[{w:'stamps',e:'邮票 ✉️+s',s1:'/m/',s2:'/p/',s3:'/s/',l1:'m',l2:'p',l3:'s'},{w:'plants',e:'植物 🌱+s',s1:'/n/',s2:'/t/',s3:'/s/',l1:'n',l2:'t',l3:'s'},{w:'trusts',e:'信任 🤝+s',s1:'/s/',s2:'/t/',s3:'/s/',l1:'s',l2:'t',l3:'s'},{w:'blasts',e:'爆炸 💥+s',s1:'/s/',s2:'/t/',s3:'/s/',l1:'s',l2:'t',l3:'s'},{w:'drinks',e:'饮料 🥤+s',s1:'/ŋ/',s2:'/k/',s3:'/s/',l1:'n',l2:'k',l3:'s'},{w:'twists',e:'扭转 🪢+s',s1:'/s/',s2:'/t/',s3:'/s/',l1:'s',l2:'t',l3:'s'},{w:'blanks',e:'空白 ⬜+s',s1:'/ŋ/',s2:'/k/',s3:'/s/',l1:'n',l2:'k',l3:'s'},{w:'trunks',e:'树干 🧳+s',s1:'/ŋ/',s2:'/k/',s3:'/s/',l1:'n',l2:'k',l3:'s'}];
  var h='';
  c.forEach(function(d){h+='<div style="background:rgba(255,138,101,.04);border:1px solid rgba(255,138,101,.2);border-radius:12px;padding:9px 12px;text-align:center;min-width:80px">';h+='<div style="font-size:1.1em;font-weight:bold;color:#ff8a65;letter-spacing:2px;margin-bottom:2px">'+d.w+'</div>';h+='<div style="display:flex;gap:1px;justify-content:center;align-items:center;margin-bottom:3px;flex-wrap:wrap">';h+='<span style="color:#ef9a9a;font-weight:bold;cursor:pointer;font-size:.8em" onclick="spkPhoneme(\''+d.s1+'\',this)">'+d.l1+' 🔊</span>';h+='<span style="color:#888;font-size:.6em">+</span>';h+='<span style="color:#ef9a9a;font-weight:bold;cursor:pointer;font-size:.8em" onclick="spkPhoneme(\''+d.s2+'\',this)">'+d.l2+' 🔊</span>';h+='<span style="color:#888;font-size:.6em">+</span>';h+='<span style="color:#ef9a9a;font-weight:bold;cursor:pointer;font-size:.8em" onclick="spkPhoneme(\''+d.s3+'\',this)">'+d.l3+' 🔊</span>';h+='</div>';h+='<div style="font-size:.6em;color:#aaa">'+d.e+'</div>';h+='<span class="speak-btn" onclick="spk(\''+d.w+'\')" style="display:inline-block;margin-top:2px;width:24px;height:24px;font-size:.55em">🔊</span></div>'});
  document.getElementById('ccvcccCards').innerHTML=h;
})();
(function(){
  var c=[{w:'sprint',e:'冲刺 🏃',s1:'/n/',s2:'/t/',l1:'n',l2:'t'},{w:'strand',e:'一缕 🧵',s1:'/n/',s2:'/d/',l1:'n',l2:'d'},{w:'strict',e:'严格 📏',s1:'/k/',s2:'/t/',l1:'c',l2:'t'},{w:'script',e:'剧本 📜',s1:'/p/',s2:'/t/',l1:'p',l2:'t'},{w:'splint',e:'夹板 🩹',s1:'/n/',s2:'/t/',l1:'n',l2:'t'},{w:'scrimp',e:'节省 🪙',s1:'/m/',s2:'/p/',l1:'m',l2:'p'}];
  var h='';
  c.forEach(function(d){h+='<div style="background:rgba(206,147,216,.04);border:1px solid rgba(206,147,216,.2);border-radius:12px;padding:9px 12px;text-align:center;min-width:80px">';h+='<div style="font-size:1.1em;font-weight:bold;color:#ce93d8;letter-spacing:2px;margin-bottom:2px">'+d.w+'</div>';h+='<div style="display:flex;gap:1px;justify-content:center;align-items:center;margin-bottom:3px;flex-wrap:wrap">';h+='<span style="color:#ef9a9a;font-weight:bold;cursor:pointer;font-size:.8em" onclick="spkPhoneme(\''+d.s1+'\',this)">'+d.l1+' 🔊</span>';h+='<span style="color:#888;font-size:.6em">+</span>';h+='<span style="color:#ef9a9a;font-weight:bold;cursor:pointer;font-size:.8em" onclick="spkPhoneme(\''+d.s2+'\',this)">'+d.l2+' 🔊</span>';h+='</div>';h+='<div style="font-size:.6em;color:#aaa">'+d.e+'</div>';h+='<span class="speak-btn" onclick="spk(\''+d.w+'\')" style="display:inline-block;margin-top:2px;width:24px;height:24px;font-size:.55em">🔊</span></div>'});
  document.getElementById('cccvccCards').innerHTML=h;
})();
(function(){
  var c=[{w:'sprints',e:'冲刺 🏃+s',s1:'/n/',s2:'/t/',s3:'/s/',l1:'n',l2:'t',l3:'s'},{w:'strands',e:'一缕 🧵+s',s1:'/n/',s2:'/d/',s3:'/z/',l1:'n',l2:'d',l3:'s'},{w:'scripts',e:'剧本 📜+s',s1:'/p/',s2:'/t/',s3:'/s/',l1:'p',l2:'t',l3:'s'},{w:'splints',e:'夹板 🩹+s',s1:'/n/',s2:'/t/',s3:'/s/',l1:'n',l2:'t',l3:'s'},{w:'scrimps',e:'节省 🪙+s',s1:'/m/',s2:'/p/',s3:'/s/',l1:'m',l2:'p',l3:'s'}];
  var h='';
  c.forEach(function(d){h+='<div style="background:rgba(239,154,154,.04);border:1px solid rgba(239,154,154,.2);border-radius:12px;padding:9px 12px;text-align:center;min-width:80px">';h+='<div style="font-size:1.1em;font-weight:bold;color:#ef9a9a;letter-spacing:2px;margin-bottom:2px">'+d.w+'</div>';h+='<div style="display:flex;gap:1px;justify-content:center;align-items:center;margin-bottom:3px;flex-wrap:wrap">';h+='<span style="color:#ef9a9a;font-weight:bold;cursor:pointer;font-size:.8em" onclick="spkPhoneme(\''+d.s1+'\',this)">'+d.l1+' 🔊</span>';h+='<span style="color:#888;font-size:.6em">+</span>';h+='<span style="color:#ef9a9a;font-weight:bold;cursor:pointer;font-size:.8em" onclick="spkPhoneme(\''+d.s2+'\',this)">'+d.l2+' 🔊</span>';h+='<span style="color:#888;font-size:.6em">+</span>';h+='<span style="color:#ef9a9a;font-weight:bold;cursor:pointer;font-size:.8em" onclick="spkPhoneme(\''+d.s3+'\',this)">'+d.l3+' 🔊</span>';h+='</div>';h+='<div style="font-size:.6em;color:#aaa">'+d.e+'</div>';h+='<span class="speak-btn" onclick="spk(\''+d.w+'\')" style="display:inline-block;margin-top:2px;width:24px;height:24px;font-size:.55em">🔊</span></div>'});
  document.getElementById('cccvcccCards').innerHTML=h;
})();

// ═══ EASTER EGG ═══
(function(){
  var c=[{w:'shrimp',e:'虾 🦐',s:'sh',r:'r',sIPA:'/ʃ/',rIPA:'/r/',end:'mp',e1:'/m/',e2:'/p/'},{w:'thrift',e:'节俭 💰',s:'th',r:'r',sIPA:'/θ/',rIPA:'/r/',end:'ft',e1:'/f/',e2:'/t/'},{w:'thrust',e:'猛推 🚀',s:'th',r:'r',sIPA:'/θ/',rIPA:'/r/',end:'st',e1:'/s/',e2:'/t/'}];
  var h='';
  c.forEach(function(d){h+='<div style="background:rgba(255,138,101,.04);border:1px solid rgba(255,138,101,.2);border-radius:12px;padding:10px 12px;text-align:center;min-width:90px">';h+='<div style="font-size:1em;font-weight:bold;color:#ffd200;letter-spacing:1px;margin-bottom:3px">'+d.w+' '+d.e+'</div>';h+='<div style="display:flex;gap:1px;justify-content:center;align-items:center;flex-wrap:wrap;font-size:.68em;margin-bottom:2px">';h+='<span style="color:#ff8a65;font-weight:bold;cursor:pointer" onclick="spkPhoneme(\''+d.sIPA+'\',this)">'+d.s+' 🔊</span>';h+='<span style="color:#888">+</span>';h+='<span style="color:#ce93d8;font-weight:bold;cursor:pointer" onclick="spkPhoneme(\''+d.rIPA+'\',this)">'+d.r+' 🔊</span>';h+='<span style="color:#888">+V+</span>';h+='<span style="color:#ef9a9a;font-weight:bold;cursor:pointer" onclick="spkPhoneme(\''+d.e1+'\',this)">'+d.end[0]+' 🔊</span>';h+='<span style="color:#888">+</span>';h+='<span style="color:#ef9a9a;font-weight:bold;cursor:pointer" onclick="spkPhoneme(\''+d.e2+'\',this)">'+d.end[1]+' 🔊</span>';h+='</div>';h+='<div style="font-size:.55em;color:#aaa">'+d.s+'=二合音(1C)+r=1C→2C开头</div>';h+='<span class="speak-btn" onclick="spk(\''+d.w+'\')" style="display:inline-block;margin-top:3px;width:18px;height:18px;font-size:.45em">🔊</span></div>'});
  h+='<div style="background:rgba(255,210,0,.06);border:2px solid #ffd200;border-radius:12px;padding:10px 12px;text-align:center;min-width:100px;margin-top:4px">';h+='<div style="font-size:1em;font-weight:bold;color:#ffd200;letter-spacing:1px;margin-bottom:3px">sphinx 狮身人面 🗿 ⭐</div>';h+='<div style="display:flex;gap:1px;justify-content:center;align-items:center;flex-wrap:wrap;font-size:.68em;margin-bottom:2px">';h+='<span style="color:#ce93d8;font-weight:bold;cursor:pointer" onclick="spkPhoneme(\'/s/\',this)">s 🔊</span>';h+='<span style="color:#888">+</span>';h+='<span style="color:#ff8a65;font-weight:bold;cursor:pointer" onclick="spkPhoneme(\'/f/\',this)">ph 🔊</span>';h+='<span style="color:#888">+V+</span>';h+='<span style="color:#ef9a9a;font-weight:bold;cursor:pointer" onclick="spkPhoneme(\'/ŋ/\',this)">n 🔊</span>';h+='<span style="color:#888">+</span>';h+='<span style="color:#ef9a9a;font-weight:bold;cursor:pointer" onclick="spkPhoneme(\'/k/\',this)">k 🔊</span>';h+='<span style="color:#888">+</span>';h+='<span style="color:#ef9a9a;font-weight:bold;cursor:pointer" onclick="spkPhoneme(\'/s/\',this)">s 🔊</span>';h+='</div>';h+='<div style="font-size:.55em;color:#ffd200">s+ph=2C开头,ph=二合音(1C)→CCVCCC</div>';h+='<span class="speak-btn" onclick="spk(\'sphinx\')" style="display:inline-block;margin-top:3px;width:18px;height:18px;font-size:.45em">🔊</span></div>';
  document.getElementById('easterStart').innerHTML=h;
  h+='<div style="margin-top:8px;font-size:.7em;color:#aaa;text-align:center">🔹 更多（CCVC）：shred ✂️ / shrub 🌿 / shrug 🤷 / throb 💓</div>';
  document.getElementById('easterStart').innerHTML=h;
})();
(function(){
  var c=[{w:'crunch',e:'嘎吱 🦷',sb:'cr',si:['/k/','/r/'],c:'n',cIPA:'/n/',dg:'ch',dIPA:'/tʃ/'},{w:'branch',e:'树枝 🌿',sb:'br',si:['/b/','/r/'],c:'n',cIPA:'/n/',dg:'ch',dIPA:'/tʃ/'},{w:'drench',e:'湿透 💧',sb:'dr',si:['/d/','/r/'],c:'n',cIPA:'/n/',dg:'ch',dIPA:'/tʃ/'},{w:'stench',e:'臭味 👃',sb:'st',si:['/s/','/t/'],c:'n',cIPA:'/n/',dg:'ch',dIPA:'/tʃ/'},{w:'trench',e:'沟渠 🕳',sb:'tr',si:['/t/','/r/'],c:'n',cIPA:'/n/',dg:'ch',dIPA:'/tʃ/'}];
  var h='';
  c.forEach(function(d){h+='<div style="background:rgba(255,138,101,.04);border:1px solid rgba(255,138,101,.2);border-radius:12px;padding:10px 12px;text-align:center;min-width:90px">';h+='<div style="font-size:1em;font-weight:bold;color:#ffd200;letter-spacing:1px;margin-bottom:3px">'+d.w+' '+d.e+'</div>';h+='<div style="display:flex;gap:1px;justify-content:center;align-items:center;flex-wrap:wrap;font-size:.68em;margin-bottom:2px">';h+='<span style="color:#ce93d8;font-weight:bold;cursor:pointer" onclick="spkPhoneme(\''+d.si[0]+'\',this)">'+d.sb[0]+' 🔊</span>';h+='<span style="color:#888">+</span>';h+='<span style="color:#ce93d8;font-weight:bold;cursor:pointer" onclick="spkPhoneme(\''+d.si[1]+'\',this)">'+d.sb[1]+' 🔊</span>';h+='<span style="color:#888">+V+</span>';h+='<span style="color:#ef9a9a;font-weight:bold;cursor:pointer" onclick="spkPhoneme(\''+d.cIPA+'\',this)">'+d.c+' 🔊</span>';h+='<span style="color:#888">+</span>';h+='<span style="color:#ff8a65;font-weight:bold;cursor:pointer" onclick="spkPhoneme(\''+d.dIPA+'\',this)">'+d.dg+' 🔊</span>';h+='</div>';h+='<div style="font-size:.55em;color:#aaa">n+ch=2C, ch=二合音(1C)</div>';h+='<span class="speak-btn" onclick="spk(\''+d.w+'\')" style="display:inline-block;margin-top:3px;width:18px;height:18px;font-size:.45em">🔊</span></div>'});
  h+='<div style="background:rgba(255,138,101,.04);border:1px solid rgba(255,138,101,.2);border-radius:12px;padding:10px 12px;text-align:center;min-width:90px">';h+='<div style="font-size:1em;font-weight:bold;color:#ffd200;letter-spacing:1px;margin-bottom:3px">squelch 压制 💦</div>';h+='<div style="display:flex;gap:1px;justify-content:center;align-items:center;flex-wrap:wrap;font-size:.68em;margin-bottom:2px">';h+='<span style="color:#ce93d8;font-weight:bold">squ</span><span style="color:#888">+V+</span>';h+='<span style="color:#ef9a9a;font-weight:bold;cursor:pointer" onclick="spkPhoneme(\'/l/\',this)">l 🔊</span>';h+='<span style="color:#888">+</span>';h+='<span style="color:#ff8a65;font-weight:bold;cursor:pointer" onclick="spkPhoneme(\'/tʃ/\',this)">ch 🔊</span>';h+='</div>';h+='<div style="font-size:.55em;color:#aaa">squ=3C+lch=2C→CCCVCC</div>';h+='<span class="speak-btn" onclick="spk(\'squelch\')" style="display:inline-block;margin-top:3px;width:18px;height:18px;font-size:.45em">🔊</span></div>';
  h+='<div style="background:rgba(255,210,0,.06);border:2px solid #ffd200;border-radius:12px;padding:10px 12px;text-align:center;min-width:100px;margin-top:4px">';h+='<div style="font-size:1em;font-weight:bold;color:#ffd200;letter-spacing:1px;margin-bottom:3px">strength 力量 💪 ⭐</div>';h+='<div style="display:flex;gap:1px;justify-content:center;align-items:center;flex-wrap:wrap;font-size:.68em;margin-bottom:2px">';h+='<span style="color:#ce93d8;font-weight:bold">str</span><span style="color:#888">+V+</span>';h+='<span style="color:#ff8a65;font-weight:bold;cursor:pointer" onclick="spkPhoneme(\'/ŋ/\',this)">ng 🔊</span>';h+='<span style="color:#888">+</span>';h+='<span style="color:#ff8a65;font-weight:bold;cursor:pointer" onclick="spkPhoneme(\'/θ/\',this)">th 🔊</span>';h+='</div>';h+='<div style="font-size:.55em;color:#ffd200">双重二合音！ng+th=2C</div>';h+='<span class="speak-btn" onclick="spk(\'strength\')" style="display:inline-block;margin-top:3px;width:18px;height:18px;font-size:.45em">🔊</span></div>';
  document.getElementById('easterEnd').innerHTML=h;
  h+='<div style="margin-top:8px;font-size:.7em;color:#aaa;text-align:center">🔹 更多：clench 握紧 ✊ (cl+ench)</div>';
  document.getElementById('easterEnd').innerHTML=h;
})();

// ═══ LAB ═══
var curStruct='ccvcc',curSub='nd';
var labCCVCC={
  'nd':{el:'nd',words:[['stand','站立 🧍'],['spend','花费 💸'],['bland','平淡 😐'],['blend','混合 🌀'],['brand','品牌 🏷'],['blond','金发 👱'],['grand','宏伟 🏰']]},
  'nt':{el:'nt',words:[['spent','花费过 💸'],['front','前面 🚪'],['grant','授予 🎫'],['plant','植物 🌱'],['slant','倾斜 📐'],['blunt','钝的 🔪'],['grunt','咕哝 😤'],['stunt','特技 🎪']]},
  'mp':{el:'mp',words:[['stamp','邮票 ✉️'],['clamp','夹子 🔧'],['cramp','抽筋 😫'],['tramp','流浪者 🚶'],['plump','丰满 🍗'],['slump','暴跌 📉'],['trump','王牌 ♠️'],['stomp','跺脚 👣'],['crimp','卷曲 🦐'],['skimp','吝啬 🪙'],['primp','打扮 💄'],['frump','邋遢 👗']]},
  'nk':{el:'nk',words:[['blank','空白 ⬜'],['plank','木板 🪵'],['prank','恶作剧 🎭'],['clank','叮当 🔗'],['crank','曲柄 🔧'],['stink','臭味 🤢'],['blink','眨眼 👁'],['brink','边缘 🌊'],['drink','喝 🥤'],['clink','碰杯 🥂'],['trunk','树干 🧳'],['flunk','不及格 📉'],['slink','溜走 🐍'],['spunk','勇气 💪'],['plunk','弹拨 🪕']]},
  'stftct':{el:'__',words:[['blast','爆炸 💥'],['crest','顶峰 🏔'],['crust','外壳 🍞'],['frost','霜 ❄️'],['trust','信任 🤝'],['twist','扭转 🪢'],['grist','谷物 🌾'],['draft','草稿 📝'],['drift','漂流 🌊'],['tract','地带 📄'],['crept','爬行 🐢']]},
  'spsklt':{el:'__',words:[['clasp','扣子 🔒'],['grasp','抓住 ✊'],['brisk','轻快 🏃'],['flask','烧瓶 🧪'],['stilt','高跷 🪜'],['scalp','头皮 💆'],['skulk','躲藏 🕵'],['scald','烫伤 🔥'],['smelt','冶炼 🔩'],['swift','快速 💨'],['dwelt','住过 🏠'],['glint','闪光 ✨']]}
};
var labCCVCCC={el:'__',words:[['stamps','邮票 ✉️+s'],['plants','植物 🌱+s'],['drinks','饮料 🥤+s'],['trusts','信任 🤝+s'],['blasts','爆炸 💥+s'],['frosts','霜 ❄️+s'],['twists','扭转 🪢+s'],['grants','授予 🎫+s'],['blanks','空白 ⬜+s'],['blinks','眨眼 👁+s'],['clamps','夹子 🔧+s'],['cramps','抽筋 😫+s'],['stomps','跺脚 👣+s'],['grasps','抓住 ✊+s'],['drafts','草稿 📝+s'],['drifts','漂流 🌊+s'],['stunts','特技 🎪+s'],['trunks','树干 🧳+s']]};
var labCCCVCC={el:'__',words:[['splint','夹板 🩹 spl+nt'],['sprint','冲刺 🏃 spr+nt'],['strand','一缕 🧵 str+nd'],['strict','严格 📏 str+ct'],['script','剧本 📜 scr+pt'],['scrimp','节省 🪙 scr+mp']]};
var labCCCVCCC={el:'__',words:[['splints','夹板 🩹+s'],['sprints','冲刺 🏃+s'],['strands','一缕 🧵+s'],['scripts','剧本 📜+s'],['scrimps','节省 🪙+s']]};

var structLabels={ccvcc:'CCVCC (2+1+2)',ccvccc:'CCVCCC (2+1+3)',cccvcc:'CCCVCC (3+1+2)',cccvccc:'CCCVCCC (3+1+3)'};
var ccvccSubs=['nd','nt','mp','nk','stftct','spsklt'];
var ccvccSubLabels={nd:'-nd',nt:'-nt',mp:'-mp',nk:'-nk',stftct:'-st/-ft/-ct',spsklt:'-sp/-sk/-lt'};

function switchStruct(s){curStruct=s;if(s==='ccvcc')curSub='nd';buildL()}
function switchSub(s){curSub=s;buildL()}
function buildL(){
  var tabs='';
  Object.keys(structLabels).forEach(function(k){tabs+='<button class="lab-tab'+(k===curStruct?' active':'')+'" onclick="switchStruct(\''+k+'\')">'+structLabels[k]+'</button>'});
  document.getElementById('labTabs').innerHTML=tabs;
  var subtabs='';
  if(curStruct==='ccvcc'){ccvccSubs.forEach(function(k){subtabs+='<button class="lab-subtab'+(k===curSub?' active':'')+'" onclick="switchSub(\''+k+'\')">'+ccvccSubLabels[k]+'</button>'})}
  document.getElementById('labSubtabs').innerHTML=subtabs;
  var words;
  if(curStruct==='ccvcc'){words=labCCVCC[curSub].words}
  else if(curStruct==='ccvccc'){words=labCCVCCC.words}
  else if(curStruct==='cccvcc'){words=labCCCVCC.words}
  else{words=labCCCVCCC.words}
  var vowels='aeiou';
  document.getElementById('labGrid').innerHTML=words.map(function(w,i){
    ...
    var ebLen=(curStruct==='ccvccc'||curStruct==='cccvccc')?3:2;
    var ebPos=len-ebLen;
    var j=0,colored='';
    while(j<len){
    ...
    return '<div class="lab-card" id="lc'+i+'" onclick="document.getElementById(\'lc'+i+'\').classList.toggle(\'flipped\');spk(\''+word+'\')"><div class="word" style="letter-spacing:3px">'+colored+'</div><div class="hint">'+emoji+'</div></div>';
  }).join('');
}

// ═══ SORT ═══
var sortWords=[{w:'stand',t:'ccvcc'},{w:'spend',t:'ccvcc'},{w:'blend',t:'ccvcc'},{w:'grand',t:'ccvcc'},{w:'brand',t:'ccvcc'},{w:'grant',t:'ccvcc'},{w:'plant',t:'ccvcc'},{w:'front',t:'ccvcc'},{w:'blunt',t:'ccvcc'},{w:'grunt',t:'ccvcc'},{w:'stamp',t:'ccvcc'},{w:'clamp',t:'ccvcc'},{w:'trump',t:'ccvcc'},{w:'stomp',t:'ccvcc'},{w:'blank',t:'ccvcc'},{w:'drink',t:'ccvcc'},{w:'trunk',t:'ccvcc'},{w:'stink',t:'ccvcc'},{w:'trust',t:'ccvcc'},{w:'blast',t:'ccvcc'},{w:'frost',t:'ccvcc'},{w:'tract',t:'ccvcc'},{w:'clasp',t:'ccvcc'},{w:'flask',t:'ccvcc'},{w:'scalp',t:'ccvcc'},{w:'swift',t:'ccvcc'},{w:'glint',t:'ccvcc'},{w:'stamps',t:'ccvccc'},{w:'plants',t:'ccvccc'},{w:'drinks',t:'ccvccc'},{w:'trusts',t:'ccvccc'},{w:'blasts',t:'ccvccc'},{w:'trunks',t:'ccvccc'},{w:'stunts',t:'ccvccc'},{w:'splint',t:'cccvcc'},{w:'sprint',t:'cccvcc'},{w:'strand',t:'cccvcc'},{w:'strict',t:'cccvcc'},{w:'script',t:'cccvcc'},{w:'scrimp',t:'cccvcc'},{w:'sprints',t:'cccvccc'},{w:'strands',t:'cccvccc'},{w:'scripts',t:'cccvccc'}];
...
function buildSort(){
  ...
  pool.innerHTML=h;
  ['CCVCC','CCVCCC','CCCVCC','CCCVCCC'].forEach(function(v){document.getElementById('bin'+v+'Items').innerHTML=''});
  document.getElementById('sortFeedback').innerHTML='';selectedWord=null;placedCount=0;setupBins();
}
function setupBins(){['binCCVCC','binCCVCCC','binCCCVCC','binCCCVCCC'].forEach(function(id){var el=document.getElementById(id);el.ondragover=function(e){e.preventDefault();el.classList.add('drag-over')};el.ondragleave=function(){el.classList.remove('drag-over')};el.ondrop=function(e){e.preventDefault();el.classList.remove('drag-over');var w=e.dataTransfer.getData('text');if(w)dropToBin(w,id.replace('bin','').toLowerCase())}})}
...
function dropToBin(w,type,correct){if(!correct){var it=document.querySelector('.sort-item[data-word="'+w+'"].unplaced');correct=it?it.getAttribute('data-type'):null}var ok=type===correct;var item=document.querySelector('.sort-item[data-word="'+w+'"].unplaced');if(!item)return;if(ok){item.classList.remove('unplaced','selected');item.classList.add('placed');item.draggable=false;item.onclick=null;item.style.cursor='default';document.getElementById('bin'+type.toUpperCase()+'Items').innerHTML+='<span class="sort-item placed" style="margin:2px;background:rgba(105,240,174,.1);border-color:rgba(105,240,174,.4);color:#69f0ae">'+w+'</span>';document.getElementById('sortFeedback').innerHTML='<span style="color:#69f0ae">✅ '+w+' → '+type.toUpperCase()+'!</span>';placedCount++;if(placedCount>=sortWords.length)document.getElementById('sortFeedback').innerHTML='<span style="color:#ffd200">🎉 全部正确！</span>'}else{item.classList.add('wrong');setTimeout(function(){item.classList.remove('wrong')},500);document.getElementById('sortFeedback').innerHTML='<span style="color:#ff5252">❌ 再听听，开头和结尾各有几个辅音？</span>'}selectedWord=null}
...
document.addEventListener('touchmove',function(e){if(!tc)return;e.preventDefault();var t=e.touches[0];tc.style.left=(t.clientX-20)+'px';tc.style.top=(t.clientY-15)+'px';['binCCVCC','binCCVCCC','binCCCVCC','binCCCVCCC'].forEach(function(id){var el=document.getElementById(id),r=el.getBoundingClientRect();if(t.clientX>=r.left&&t.clientX<=r.right&&t.clientY>=r.top&&t.clientY<=r.bottom)el.classList.add('drag-over');else el.classList.remove('drag-over')})},{passive:false});
document.addEventListener('touchend',function(e){if(!tc)return;var t=e.changedTouches[0],type=null;['binCCVCC','binCCVCCC','binCCCVCC','binCCCVCCC'].forEach(function(id){var el=document.getElementById(id),r=el.getBoundingClientRect();if(t.clientX>=r.left&&t.clientX<=r.right&&t.clientY>=r.top&&t.clientY<=r.bottom)type=id.replace('bin','').toLowerCase();el.classList.remove('drag-over')});if(type&&tl){var correct=document.querySelector('.sort-item[data-word="'+tl+'"].unplaced');if(correct)dropToBin(tl,type,correct.getAttribute('data-type'))}if(tc){document.body.removeChild(tc);tc=null}tl=null});
...
var quizPool=[{t:'struct',q:'stamp 是什么结构？',w:'stamp',o:['CCVCC','CCVCCC','CCCVCC','CCCVCCC'],a:0,x:'st(2)+a+mp(2)=CCVCC，5个音位'},{t:'struct',q:'plants 是什么结构？',w:'plants',o:['CCVCC','CCVCCC','CCCVCC','CCCVCCC'],a:1,x:'pl(2)+a+nts(3)=CCVCCC，6个音位'},{t:'struct',q:'sprint 是什么结构？',w:'sprint',o:['CCVCC','CCVCCC','CCCVCC','CCCVCCC'],a:2,x:'spr(3)+i+nt(2)=CCCVCC，6个音位'},{t:'struct',q:'sprints 是什么结构？',w:'sprints',o:['CCVCC','CCVCCC','CCCVCC','CCCVCCC'],a:3,x:'spr(3)+i+nts(3)=CCCVCCC，7个音位'},{t:'struct',q:'drink 是什么结构？',w:'drink',o:['CCVCC','CCVCCC','CCCVCC','CCCVCCC'],a:0,x:'dr(2)+i+nk(2)=CCVCC，5个音位'},{t:'struct',q:'trusts 是什么结构？',w:'trusts',o:['CCVCC','CCVCCC','CCCVCC','CCCVCCC'],a:1,x:'tr(2)+u+sts(3)=CCVCCC'},{t:'struct',q:'strand 是什么结构？',w:'strand',o:['CCVCC','CCVCCC','CCCVCC','CCCVCCC'],a:2,x:'str(3)+a+nd(2)=CCCVCC'},{t:'struct',q:'strict 是什么结构？',w:'strict',o:['CCVCC','CCVCCC','CCCVCC','CCCVCCC'],a:2,x:'str(3)+i+ct(2)=CCCVCC。ct=2个音'},{t:'struct',q:'stop 是什么结构？',w:'stop',o:['CCVCC','CCVC','CVCC','CCCVCC'],a:1,x:'st(2)+o+p(1)=CCVC。只有开头连缀'},{t:'struct',q:'hand 是什么结构？',w:'hand',o:['CCVCC','CCVC','CVCC','CCCVCC'],a:2,x:'h(1)+a+nd(2)=CVCC。只有结尾连缀'},{t:'count',q:'🔊 听发音，sprint 有几个音位？',w:'sprint',o:['4个','5个','6个','7个'],a:2,x:'=6个音位'},{t:'count',q:'🔊 听发音，plants 有几个音位？',w:'plants',o:['4个','5个','6个','7个'],a:2,x:'=6个音位'},{t:'count',q:'🔊 听发音，sprints 有几个音位？',w:'sprints',o:['5个','6个','7个','8个'],a:2,x:'=7个音位'},{t:'count',q:'🔊 听发音，stamp 有几个音位？',w:'stamp',o:['3个','4个','5个','6个'],a:2,x:'=5个音位'},{t:'listen',q:'🔊 开头有几个辅音？',w:'sprint',o:['1个','2个','3个','4个'],a:2,x:'spr=3个辅音'},{t:'listen',q:'🔊 开头有几个辅音？',w:'stamp',o:['1个','2个','3个','4个'],a:1,x:'st=2个辅音'},{t:'listen',q:'🔊 结尾有几个辅音？',w:'plants',o:['1个','2个','3个','4个'],a:2,x:'nts=3个辅音'},{t:'vowel',q:'stamp 中间的元音是什么？',w:'stamp',o:['a /æ/ (短)','a /eɪ/ (长)','e /e/','i /ɪ/'],a:0,x:'a 读短音 /æ/'},{t:'vowel',q:'sprint 中间的元音是什么？',w:'sprint',o:['i /aɪ/ (长)','i /ɪ/ (短)','e /e/','a /æ/'],a:1,x:'i 读短音 /ɪ/'},{t:'choose',q:'下面哪个是 CCCVCC？',o:['stamp','drink','sprint','trusts'],a:2,x:'sprint=spr+int'},{t:'choose',q:'下面哪个是 CCVCCC？',o:['stamp','plants','sprint','drink'],a:1,x:'plants=pl+ants'},{t:'choose',q:'下面哪个是 CCCVCCC？',o:['stamp','plants','sprint','sprints'],a:3,x:'sprints=spr+ints'},{t:'choose',q:'哪个不是混合连缀？',o:['stamp','cat','drink','trust'],a:1,x:'cat=CVC'},{t:'spell',q:'/sprɪnt/ 正确拼写？',o:['sprint','sprinnt','sprind','sprintt'],a:0,x:'s-p-r-i-n-t=sprint'},{t:'spell',q:'/strænd/ 正确拼写？',o:['strand','strannd','stranned','strandd'],a:0,x:'s-t-r-a-n-d=strand'},{t:'digraph',q:'shrimp 开头 shr 有几个音位？',w:'shrimp',o:['1个','2个','3个','4个'],a:1,x:'sh=二合音(1C)+r=1C=2C'},{t:'digraph',q:'branch 结尾 -nch 有几个音位？',w:'branch',o:['1个','2个','3个','4个'],a:1,x:'n=1C+ch=二合音(1C)=2C'}];
var qS={qs:[],cur:0,as:[]};
