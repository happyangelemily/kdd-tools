...
function playAudio(path,el){if(!el||!el.classList)return;if(el.classList.contains('on'))return;var a=audioCache[path];var src=a?a.src:(AUDIO_BASE+path);var player=new Audio(src);player.preload='auto';el.classList.add('on');player.onended=function(){el.classList.remove('on')};player.onerror=function(){el.classList.remove('on')};player.play().catch(function(e){el.classList.remove('on')})}
...
function spk(word){
  var w=word.toLowerCase();var p=WORDS[w];
  if(p){
    var a=audioCache[p];
    var src=a?a.src:(AUDIO_BASE+p);
    var player=new Audio(src);
    player.preload='auto';
    var fired=false;
    var fallback=function(){if(!fired){fired=true;speakNow(w)}};
